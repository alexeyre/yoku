...
mod tests {
    use super::*;
    use crate::db::models::{Exercise as Ex, Muscle as Mu};

    #[tokio::test]
    async fn connect_and_seed() {
        // This test only runs when a neo4j instance is available in env config.
        // It validates that the functions execute without panicking.
        if env::var("NEO4J_HOST").is_err() {
            // skip if no config
            return;
        }

        let gm = GraphManager::connect().await.expect("connect");
        gm.seed_defaults().await.expect("seed");
    }
    ...
    async fn upsert_nodes() {
        ...
        let mu = Mu {
            id: uuid::Uuid::new_v4(),
            name: "test-muscle".into(),
            created_at: chrono::Utc::now(),
            updated_at: chrono::Utc::now(),
        };

        gm.upsert_exercise(&ex).await.expect("upsert_ex");
        gm.upsert_muscle(&mu).await.expect("upsert_m");
        gm.upsert_exercise_muscle(&ex, &mu, "primary")
            .await
            .expect("link");
    }
}
